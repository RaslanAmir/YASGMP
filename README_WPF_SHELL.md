...
- **Configuration flow:** The integration relies on the existing `DatabaseService` connection string and the shell session defaults documented above. When the shell starts it hydrates `UserSession` using `Shell:UserId`, `Shell:Username`, and `Shell:FullName`, so the cockpit immediately has a numeric `UserId` to send into `GetKpiWidgetsAsync(userId, …)` until the operator signs in.【F:YasGMP.Wpf/Services/UserSession.cs†L17-L59】【F:YasGMP.Wpf/ViewModels/CockpitViewModel.cs†L137-L169】 No additional configuration keys are required beyond keeping the database connection accurate.
- **Realtime/session hooks:** `DashboardModuleViewModel` subscribes to `ISignalRClientService.AuditReceived` and triggers `RefreshAsync()` whenever the service pushes a new audit event, keeping the WPF dashboard synchronized with server-side updates.【F:YasGMP.Wpf/ViewModels/Modules/DashboardModuleViewModel.cs†L27-L124】 `CockpitViewModel` listens for `ILocalizationService.LanguageChanged` and `UserSession.SessionChanged`, reloading metrics whenever the operator changes language or switches accounts so the localized `Cockpit.Status.*` strings and KPI totals stay current.【F:YasGMP.Wpf/ViewModels/CockpitViewModel.cs†L63-L116】【F:YasGMP.Wpf/ViewModels/CockpitViewModel.cs†L220-L277】
...
### Admin settings editor (form modes, validation, and save alerts)

- **Form modes:** The editor inherits the SAP B1 form-mode state machine (`Find`, `Add`, `View`, `Update`) from `DataDrivenModuleDocumentViewModel`. Entering **Add** spins up a fresh `EditableSetting` instance, while leaving edit modes clears `IsMarkedForDeletion` so delete flags do not leak back into read-only states.【F:YasGMP.Wpf/ViewModels/Modules/AdminModuleViewModel.cs†L362-L404】 The XAML binds every input’s `IsReadOnly`/`IsEnabled` to `IsInEditMode`, ensuring keys, values, categories, descriptions, and delete toggles only activate in **Add** or **Update** mode, mirroring SAP B1 interactions for QA scripts and operator training.【F:YasGMP.Wpf/Views/AdminModuleView.xaml†L112-L214】
- **Editable fields:** Operators maintain the setting key, value, category, description, and a “Marked for Deletion” flag. Categories are populated from the shared lookup collection so test scripts can validate expected options (General/System/Notifications).【F:YasGMP.Wpf/ViewModels/Modules/AdminModuleViewModel.cs†L75-L162】【F:YasGMP.Wpf/Views/AdminModuleView.xaml†L150-L214】
- **Validation rules:** Saves require a selected record; the view-model applies localized validation for missing key/value/category fields and rejects duplicate keys by normalizing whitespace and cross-checking both persisted rows and other pending edits. QA should expect inline validation messages plus a warning toast whenever validation fails outside edit mode.【F:YasGMP.Wpf/ViewModels/Modules/AdminModuleViewModel.cs†L398-L531】【F:YasGMP.Wpf/Views/AdminModuleView.xaml†L216-L260】
- **Save & delete behavior:** Save honors the delete toggle when present, issuing signature-backed deletes and upserts via `DatabaseService.UpsertSettingAsync`. Successes raise localized status text, toast alerts, and audit entries that include actor/session metadata; persistence or audit failures surface error alerts without clearing the dirty state so operators can retry after remediation.【F:YasGMP.Wpf/ViewModels/Modules/AdminModuleViewModel.cs†L532-L909】
- **Alerts:** Validation failures publish warning alerts, persistence errors publish error alerts, and successful creates/updates/deletes publish success alerts alongside the status bar text. Training material should instruct operators to check both the inline validation panel and the toast overlay after each save.【F:YasGMP.Wpf/ViewModels/Modules/AdminModuleViewModel.cs†L398-L909】【F:YasGMP.Wpf/ViewModels/ToastNotificationViewModel.cs†L9-L66】
