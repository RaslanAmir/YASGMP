...
namespace YasGMP.Services
{
    public static class DatabaseServiceMachinesCoreExtensions
    {
        ...
        {
            const string sql = @"SELECT id, code, name, description, model, manufacturer, location,
...
FROM machines WHERE id=@id LIMIT 1";
            ...
            CancellationToken token = default)
        {
            ...
            string insert = @"INSERT INTO machines (code, name, description, model, manufacturer, location, install_date, procurement_date, status, urs_doc, serial_number, acquisition_cost, rfid_tag, qr_code, iot_device_id, cloud_device_guid)
                             VALUES (@code,@name,@desc,@model,@mf,@loc,@inst,@proc,@status,@urs,@sn,@cost,@rfid,@qr,@iot,@cloud)";
            string updateSql = @"UPDATE machines SET code=@code, name=@name, description=@desc, model=@model, manufacturer=@mf, location=@loc, install_date=@inst, procurement_date=@proc, status=@status, urs_doc=@urs, serial_number=@sn, acquisition_cost=@cost, rfid_tag=@rfid, qr_code=@qr, iot_device_id=@iot, cloud_device_guid=@cloud WHERE id=@id";
            ...
            await db.LogSystemEventAsync(actorUserId, update ? "MACHINE_UPDATE" : "MACHINE_CREATE", "machines", "MachineModule", m.Id, m.Name, ip, "audit", deviceInfo, sessionId, token: token).ConfigureAwait(false);
            return m.Id;
        }
            ...
            CancellationToken token = default)
        {
            ...
            await db.LogSystemEventAsync(actorUserId, "MACHINE_EXPORT", "machines", "MachineModule", null, $"fmt={format}; file={path}; count={count}", ip, "info", deviceInfo, sessionId, token: token).ConfigureAwait(false);
            return path;
        }
            ...
            CancellationToken token = default)
        {
            try
            {
                await db.ExecuteNonQueryAsync("DELETE FROM machines WHERE id=@id", new[] { new MySqlConnector.MySqlParameter("@id", machineId) }, token).ConfigureAwait(false);
            }
            catch { /* ignore if table missing */ }
            await db.LogSystemEventAsync(actorUserId, "MACHINE_DELETE", "machines", "MachineModule", machineId, null, ip, "audit", deviceInfo, sessionId, token: token).ConfigureAwait(false);
        }
    }
}
