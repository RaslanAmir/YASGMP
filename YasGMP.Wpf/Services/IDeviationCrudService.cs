...
/// <summary>
/// Shared contract that routes deviation CRUD operations through <see cref="YasGMP.Services.DeviationService"/> and the shared MAUI audit pipeline.
/// </summary>
/// <remarks>
/// Module view models call these members on the dispatcher thread, adapters forward the work to the shared
/// <see cref="YasGMP.Services.DeviationService"/> and <see cref="YasGMP.Services.AuditService"/>, and callers marshal UI updates with
...
/// </remarks>
public interface IDeviationCrudService
{
    Task<Deviation?> TryGetByIdAsync(int id);

    /// <summary>
    /// Persists a new deviation record and returns the saved identifier with signature metadata.
    /// </summary>
    Task<CrudSaveResult> CreateAsync(Deviation deviation, DeviationCrudContext context);

    /// <summary>
    /// Updates an existing deviation record and returns the signature metadata captured during persistence.
    /// </summary>
    Task<CrudSaveResult> UpdateAsync(Deviation deviation, DeviationCrudContext context);

    void Validate(Deviation deviation);

    string NormalizeStatus(string? status);
}

/// <summary>
/// Captures the authenticated context required when persisting deviation changes. Each value flows into
/// <see cref="CrudSaveResult.SignatureMetadata"/> via <see cref="SignatureMetadataDto"/> so audit and compliance pipelines receive the accepted signature manifest.
/// </summary>
/// <remarks>
/// Adapters materialize <see cref="SignatureMetadataDto"/> from this context before returning <see cref="CrudSaveResult"/>.
/// Consumers in the WPF shell must persist the DTO alongside deviation records and surface the signature manifest in inspection panes, while MAUI screens should
/// propagate the same payload when displaying or synchronizing the record to keep the shared audit history aligned with the mobile shell and localization services.
/// </remarks>
...
public readonly record struct DeviationCrudContext(
    int UserId,
    string Ip,
    string DeviceInfo,
    string? SessionId,
    ...
    string? SignatureNote)
{
    ...
    public static DeviationCrudContext Create(int userId, string ip, string deviceInfo, string? sessionId)
        => new(
            userId <= 0 ? 1 : userId,
    ...
    public static DeviationCrudContext Create(
        int userId,
        string ip,
        string deviceInfo,
        string? sessionId,
