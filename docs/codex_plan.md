...
- [x] WPF builds *(dotnet build YasGMP.Wpf/YasGMP.Wpf.csproj succeeds; project now targets Microsoft.NET.Sdk with <UseWPF>true</UseWPF>, AES-GCM tag size pinned, and doc/nullability cleanup in place while legacy signature warnings remain on AppCore services). Also eliminated startup binding warnings by binding `DataContext` for `ModulesPane`/`InspectorPane` in `MainWindow.xaml` to `ModulesPane`/`InspectorPane` respectively (prevents early binding attempts against `MainWindowViewModel`).*
...
- **B1 - Shell foundation** (Ribbon, Docking, StatusBar, FormMode state machine) - [ ] todo
  - 2025-10-13: Extended StatusBar to show UTC, Env, Server, DB, User, and smoke status with EN↔HR labels and a11y names; bound to a 1s UTC timer.
  - 2025-10-14: Fixed early inspector/modules binding errors: `MainWindow.xaml` now sets `DataContext` for `shell:ModulesPane` and `shell:InspectorPane` to the corresponding child VMs. This removes `BindingExpression path error: 'Groups'/'ModuleTitle'/'RecordTitle'/'Fields'` spam during startup and stabilizes pane initialization before `OnLoaded` wires them.
  - 2025-10-14: Added global crash logging hooks in `App.xaml.cs` (DispatcherUnhandledException/AppDomain.UnhandledException/TaskScheduler.UnobservedTaskException) writing to `%LOCALAPPDATA%/YasGMP/logs/wpf-crash-*.txt` to capture first-chance XAML/initialization failures in user environments.
  - 2025-10-14: Eliminated cross-thread UI updates in `B1FormDocumentViewModel` (removed `ConfigureAwait(false)` on UI-affecting awaits). This stopped `NotSupportedException` from `CollectionView` and the downstream `ItemsControl` `InvalidOperationException` during initial refresh.
- 2025-10-14: Resolved AvalonDock ActiveContent binding errors by introducing `ActiveContentToDocumentConverter` and applying it to `DockingManager.ActiveContent` so Modules/Inspector panes (UserControls) no longer push back into `ActiveDocument`.
- 2025-10-14: Fixed Attachments inspector crash: `CheckBox.IsChecked` bindings for `RetentionLegalHold` and `RetentionReviewRequired` now use `Mode=OneWay` because the corresponding view-model properties are read-only (`AttachmentRowViewModel`). Prevents `InvalidOperationException` during binding activation. File: `YasGMP.Wpf/Views/AttachmentsModuleView.xaml`. Smoke passed (17/17).
- **B2 - Cross-cutting** (Attachments DB, E-Signature, Audit) - [~] in-progress *(e-sign capture now spans Assets, Components, Warehouses, Incidents, CAPA, Change Control, Validations, Scheduled Jobs, Suppliers, External Servicers, Users, and Work Orders; WPF adapters and AppCore services propagate optional signature metadata through to the database, falling back to the legacy hash generator only when metadata is missing; the WPF dialog service now emits audit events on capture/persist while broader audit surfacing remains gated on SDK access)*
    - 2025-10-07: Added XML `<summary>/<remarks>` coverage to every module view-model public surface (constants, generated properties, commands, navigation overrides) documenting form mode expectations, execution timing, and localization placeholders; Golden Arrow/CFL remarks now call out the module keys and routed status strings. `dotnet restore yasgmp.sln` succeeds, while `dotnet build yasgmp.sln` fails in `Diagnostics\\ChangeControlAssignmentHarness.cs` because the harness still references the legacy `YasGMP.ViewModels` namespace-tracked for cleanup alongside the diagnostics harness backlog.
    - 2025-10-07: Gated `Diagnostics/ChangeControlAssignmentHarness.cs` behind `YASGMP_INCLUDE_CHANGE_CONTROL_HARNESS` so default desktop builds ship the NotSupported stub without MAUI dependencies; reran `dotnet build yasgmp.sln` and the solution now completes with warning-only output.
    - 2025-10-07: Re-enabled the harness for MAUI builds by defining `YASGMP_INCLUDE_CHANGE_CONTROL_HARNESS` in `YasGMP.AppCore.csproj` (net8) and embedding a harness-local workflow that mirrors the legacy view-model assignment logic without referencing `YasGMP.ViewModels`; `dotnet build yasgmp.sln` now succeeds (warnings only) with the harness active.
    - 2025-10-07: Triaged StyleCop noise by escaping XML `<remarks>` ampersands across module view-models and normalizing alignment-heavy initializers in MAUI views (`Suppliers`, `Users`, `Validation`, `WorkOrders`), cutting dozens of CS157x/SA1025/SA1515 warnings from the default build output.
    - 2025-10-07: Unified Microsoft.Extensions.DependencyInjection packages by bumping AppCore's `Microsoft.Extensions.DependencyInjection.Abstractions` to 9.0.3 so MAUI/WPF targets stop emitting MSB3277 assembly conflicts during solution builds.
...
- Smoke: Added strict mode via YASGMP_STRICT_SMOKE=1. New tests exercise Button_Find/Button_Update in Work Orders and AttachButton in Suppliers/Work Orders (AutomationId-driven). In strict mode, asserts presence/enabled state; otherwise tolerant. Total 12 passing tests on this host.
- Smoke: Added Parts and Warehouse Attach coverage using fake file picker in smoke mode (YASGMP_SMOKE_ATTACH_FAKE=1). Save state transitions test asserts Save enabled after Add and disabled after Cancel in strict mode.
- CLI: Added --smoke to enable smoke mode and --smoke-strict for strict mode. Config: Smoke:Enabled=true, Smoke:Strict=true supported. Extended Attach smoke to Calibration/Validations with fake picker; total tests now 17 passing.
