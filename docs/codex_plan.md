...
- 2025-10-22T08:51Z: `dotnet restore yasgmp.sln`, `dotnet build yasgmp.csproj -f net9.0-windows10.0.19041.0`, and `dotnet build YasGMP.Wpf/YasGMP.Wpf.csproj -f net9.0-windows` attempted while scaffolding the Document Control module view; each command still returns `bash: command not found: dotnet` because the CLI is unavailable in the container environment.
  - 2026-05-11T09:15Z: `dotnet restore yasgmp.sln`, `dotnet build yasgmp.csproj -f net9.0-windows10.0.19041.0`, and `dotnet build YasGMP.Wpf/YasGMP.Wpf.csproj -f net9.0-windows` retried after expanding CAPA/Change Control/Incident validation coverage; each still exits with `bash: command not found: dotnet` because the CLI is unavailable in this container.
...
- [ ] Solution restores *(blocked on Windows-targeting prerequisites; historical attempts below hit missing CLI and the latest 2025-10-10 runs failed with NETSDK1100/NETSDK1147 because Windows 10 SDK and MAUI workloads are absent on linux.)*
  ...
  - 2026-05-25T09:30Z: `dotnet restore yasgmp.sln` retried for the Document Control navigation batch; CLI remains unavailable so the command exits with `bash: command not found: dotnet`.
- [ ] MAUI builds *(blocked on Windows workloads; historical attempts failed before CLI install and the latest 2025-10-10 run surfaced NETSDK1147 requiring maui-tizen workload on Windows.)*
  ...
  - 2026-05-25T09:30Z: `dotnet build yasgmp.csproj -f net9.0-windows10.0.19041.0` retried while adding Document Control ribbon/backstage commands; CLI still missing so the command exits with `bash: command not found: dotnet`.
...
- 2026-06-09T10:05Z: `dotnet restore yasgmp.sln`, `dotnet build yasgmp.csproj -f net9.0-windows10.0.19041.0`, `dotnet build YasGMP.Wpf/YasGMP.Wpf.csproj -f net9.0-windows`, and `dotnet test YasGMP.Wpf.Smoke/YasGMP.Wpf.Smoke.csproj -c Release` retried while adding SOP governance adapter regression tests; each command still returns `bash: command not found: dotnet` in this Linux container, so validation remains blocked on a Windows host.
...
- Extended the quality workspace with Training Records and SOP Governance WPF modules that wrap the shared MAUI view-models, project inspector metadata, and surface SAP B1 form-mode commands; `dotnet restore yasgmp.sln`, `dotnet build YasGMP.Wpf/YasGMP.Wpf.csproj -f net9.0-windows`, `dotnet build yasgmp.csproj -f net9.0-windows10.0.19041.0`, and `dotnet test YasGMP.Wpf.Smoke/YasGMP.Wpf.Smoke.csproj -c Release` (2026-06-08T09:05Z) still exit with `bash: command not found: dotnet` in this Linux container.
- 2026-06-09: Authored `SopGovernanceServiceAdapterTests` to cover load/create/update/delete/export flows, signature cancellation, attachment hydration/removal, and error handling; repeated the restore/build/test commands above and documented that they still fail with `bash: command not found: dotnet` pending Windows tooling.
- 2026-06-08T10:42Z: Hardened the new Training Records and SOP Governance wrapper view-models to track filtered-collection subscriptions and implement `IDisposable`, preventing stale shell event handlers from leaking after tabs close; `dotnet restore yasgmp.sln`, `dotnet build YasGMP.Wpf/YasGMP.Wpf.csproj -f net9.0-windows`, `dotnet build yasgmp.csproj -f net9.0-windows10.0.19041.0`, and `dotnet test YasGMP.Wpf.Smoke/YasGMP.Wpf.Smoke.csproj -c Release` still fail with `bash: command not found: dotnet` in this Linux container.
...
- 2026-05-25T09:30Z: `dotnet restore yasgmp.sln`, `dotnet build yasgmp.csproj -f net9.0-windows10.0.19041.0`, `dotnet build YasGMP.Wpf/YasGMP.Wpf.csproj -f net9.0-windows`, and `dotnet test YasGMP.Wpf.Smoke/YasGMP.Wpf.Smoke.csproj -c Release` retried while wiring Document Control ribbon/backstage commands; each still exits with `bash: command not found: dotnet` in this Linux container so validation remains Windows-host only.
- 2026-05-17: Landed the Deviation module view model/view with signature-aware saves, attachment uploads, CFL integration, and unit coverage mirroring incidents/change-control; retried `dotnet restore yasgmp.sln`, `dotnet build yasgmp.csproj -f net9.0-windows10.0.19041.0`, and `dotnet build YasGMP.Wpf/YasGMP.Wpf.csproj -f net9.0-windows` — each still exits with `bash: command not found: dotnet` in this Linux container.
...
- 2026-06-10: Introduced `ITrainingRecordService` and `TrainingRecordServiceAdapter` so the WPF shell enriches training record workflows with session metadata, attachment dedupe, and electronic signature capture, and refactored `TrainingRecordsModuleViewModel` to drive busy/status messaging and inspector refresh through the adapter; `dotnet restore yasgmp.sln`, `dotnet build YasGMP.Wpf/YasGMP.Wpf.csproj -f net9.0-windows`, and `dotnet build yasgmp.csproj -f net9.0-windows10.0.19041.0` still exit with `bash: command not found: dotnet` pending Windows tooling.
...
- 2025-10-22: Scaffolded the Document Control module view for the WPF shell with toolbar bindings, quality filters, a change-control picker overlay, and signature metadata so the module aligns with the MAUI client experience.
...
- **B4+ — Module rollout:**
  ...
  - Documents/Attachments — [x] done *(Attachments module now ships a finalized split inspector/editor workspace with staged upload queueing, signature-gated CRUD flow, and retention policy handling.)*
    - 2026-04-10: Document Control module now adapts the shared MAUI DocumentControlViewModel into the WPF shell with SAP B1 form-mode synchronization, inspector projections, attachment workflow plumbing, and signature capture hooks; `dotnet restore yasgmp.sln`, `dotnet build yasgmp.sln`, and `dotnet build YasGMP.Wpf/YasGMP.Wpf.csproj -f net9.0-windows` were retried but still fail with **command not found** while the dotnet CLI remains unavailable in this container.
    - 2026-05-24: WPF host now registers the shared DocumentControlViewModel, aligns ModuleRegistry metadata with ShellStrings-backed keys/descriptions, and adds neutral/en/hr Document Control resources for titles/tooltips/automation metadata; `dotnet restore yasgmp.sln`, `dotnet build YasGMP.Wpf/YasGMP.Wpf.csproj -f net9.0-windows`, and `dotnet build yasgmp.csproj -f net9.0-windows10.0.19041.0` still exit with `bash: command not found: dotnet`, leaving smoke automation pending missing CLI tooling.
    - 2026-05-25: WindowMenuViewModel now exposes `OpenDocumentControlCommand`, MainWindow.xaml wires ribbon/backstage navigation to the Document Control module with localized captions/tooltips/automation ids, and ShellStrings EN/HR gain a Segoe MDL2 document glyph; `dotnet restore yasgmp.sln`, `dotnet build yasgmp.csproj -f net9.0-windows10.0.19041.0`, `dotnet build YasGMP.Wpf/YasGMP.Wpf.csproj -f net9.0-windows`, and `dotnet test YasGMP.Wpf.Smoke/YasGMP.Wpf.Smoke.csproj -c Release` still fail with `bash: command not found: dotnet` in this container.
    - 2026-05-28: Document Control toolbar now surfaces approve/publish/expire commands backed by the service adapter with busy guards, status propagation, document reloads, and new unit tests covering success and failure paths; `dotnet restore yasgmp.sln`, `dotnet build yasgmp.sln`, and `dotnet build YasGMP.Wpf/YasGMP.Wpf.csproj -f net9.0-windows` continue to exit with `bash: command not found: dotnet`, leaving smoke automation blocked until the CLI is provisioned.
...
- 2026-05-10: CAPA, Change Control, and Incidents modules now expose Add/Approve/Execute/Close workflow commands wired through their CRUD adapters, keep editor status/records synchronized, extend fake services with transition logging, and add unit coverage for the end-to-end lifecycle; `dotnet restore`/`dotnet build`/`dotnet test` still fail with `command not found: dotnet` until the CLI is available.
- 2026-05-11: CAPA, Change Control, and Incidents module tests now assert e-signature cancellation/exception paths, adapter-surfaced validation failures, and forbidden Save/transition warnings using the enhanced fake services; latest `dotnet restore`/`dotnet build` attempts continue to fail with `bash: command not found: dotnet` pending Windows tooling.
- 2026-05-11: Added regression tests that select seeded CAPA, Change Control, and Incident records, assert inspector field projections, and verify the new timeline collections hydrate/refresh when navigation triggers adapter reloads; `dotnet restore`/`dotnet build`/`dotnet test` remain blocked in this container with `command not found: dotnet`.

- 2026-05-26: Document Control module now routes lifecycle commands, attachments, and exports through a new IDocumentControlService adapter that composes DatabaseService helpers with signature capture and manifest refresh so the WPF shell can test without the MAUI view-model; `dotnet restore yasgmp.sln`, `dotnet build YasGMP.Wpf/YasGMP.Wpf.csproj -f net9.0-windows`, and `dotnet build yasgmp.csproj -f net9.0-windows10.0.19041.0` still fail with `command not found: dotnet` in this container.
